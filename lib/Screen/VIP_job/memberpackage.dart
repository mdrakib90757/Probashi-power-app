...
class VipMemberpackage extends StatefulWidget {
  const VipMemberpackage({super.key});

  @override
  State<VipMemberpackage> createState() => _VipMemberpackageState();
}

class _VipMemberpackageState extends State<VipMemberpackage> {
  List<PackageListModel>VmemberpackageList = [];
  ...
  Future<void> fetchMemberList() async {
    ...
    if (response.statusCode == 200) {
      ...
      print(response.body);
      setState(() {
        VmemberpackageList = List<PackageListModel>.
        from(jsonData.map((item) => PackageListModel.fromJson(item)));
      });
    } else {
      print("Failed The Api ${response.body}");
    }
  }catch(e){
      print("Error${e}");
    }
          ...
          child: Padding(
            ...
            child: Column(
                children: [
                  ...
                  SizedBox(height: 10,),
                  VmemberpackageList.isEmpty?Center(
                    child: CircularProgressIndicator(),):
                  ListView.builder(
                    itemCount: VmemberpackageList.length,
                    shrinkWrap: true,
                    physics: NeverScrollableScrollPhysics(),
                    itemBuilder: (context, index) {
                      final data=VmemberpackageList[index];
                      return Container(
                        width: double.infinity,
                        ...
                        child: Column(
                          children: [
                            Row(
                              ...
                              children: [
                                Text("প্যাকেজ:${data.name}",style: TextStyle(
                                    fontFamily: "NotoSansBengali",
                                    fontSize: 15,
                                    color: Colors.black,
                                    fontWeight: FontWeight.bold
                                ),),
                                Text("মূল্য:${data.rate}",
                                  style: TextStyle(
                                      fontFamily: "NotoSansBengali",
                            ...
                            ElevatedButton(onPressed: () {
                              Navigator.push(context, MaterialPageRoute(
                                builder: (context) =>
                                    MainLayout(initialScreen: 22,selectedPackage: data,),));
                            },
