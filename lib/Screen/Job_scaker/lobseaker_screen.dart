...
class jobseakerScreen extends StatefulWidget {
  const jobseakerScreen({super.key});

  @override
  State<jobseakerScreen> createState() => _jobseakerScreenState();
}

class _jobseakerScreenState extends State<jobseakerScreen> {
  List<PackageListModel>jseackerlist=[];
  Future<void>fetchPackageList()async{
    try{
      ...
      if(responose.statusCode==200){
        ...
        setState(() {
          jseackerlist=List<PackageListModel>.
          ...
          child: Padding(
            ...
            child: Column(
                children: [
                  ...
                  SizedBox(height: 10,),
                  ListView.builder(
                    itemCount: jseackerlist.length,
                    shrinkWrap: true,
                    physics: NeverScrollableScrollPhysics(),
                    itemBuilder: (context, index) {
                      final data=jseackerlist[index];
                      return Container(
                        width: double.infinity,
                            ...
                            ElevatedButton(onPressed: () {
                              ...
                              if(authProvider.isLoggedIn){
                                Navigator.pushAndRemoveUntil(context, MaterialPageRoute(
                                    builder:(context) => MainLayout(initialScreen: 29,selectedPackage:data,)),
                                      (route) => false,
                                );
                              }else{
